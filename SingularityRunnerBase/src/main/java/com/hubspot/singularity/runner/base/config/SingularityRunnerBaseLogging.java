...
public class SingularityRunnerBaseLogging {
  ...
  public Optional<String> getRootLogPath() {
    if (primaryConfiguration.getLoggingFilename().isPresent()) {
      String directory = primaryConfiguration.getLoggingDirectory().isPresent()
      ...
      return Optional.of(Paths.get(directory).resolve(primaryConfiguration.getLoggingFilename().get()).toString());
    } else {
  ...
  public FileAppender<ILoggingEvent> buildFileAppender(LoggerContext context, String file) {
    ...
    encoder.setPattern(primaryConfiguration.getLoggingPattern().isPresent()
        ? primaryConfiguration.getLoggingPattern().get()
        : baseConfiguration.getLoggingPattern().orElse(JavaUtils.LOGBACK_LOGGING_PATTERN));
    encoder.start();
