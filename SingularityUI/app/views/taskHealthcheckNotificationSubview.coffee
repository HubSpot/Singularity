...
class taskHealthcheckNotificationSubview extends View
    ...
    renderData: =>
        ...
        healthTimeoutSeconds = if @model.get('task').taskRequest.deploy.deployHealthTimeoutSeconds then @model.get('task').taskRequest.deploy.deployHealthTimeoutSeconds else config.defaultDeployHealthTimeoutSeconds
