...
public class SingularityValidator {
    ...
    private void checkForIllegalChanges(SingularityRequest request, SingularityRequest existingRequest) {
        checkBadRequest(request.getRequestType() == existingRequest.getRequestType(),
            String.format("Request can not change requestType from %s to %s", existingRequest.getRequestType(), request.getRequestType()));
    ...
    public SingularityRequest checkSingularityRequest(SingularityRequest request, Optional<SingularityRequest> existingRequest, Optional<SingularityDeploy> activeDeploy,
        ...
        checkBadRequest(request.getInstancesSafe() <= maxInstancesPerRequest, "Instances (%s) be greater than %s (maxInstancesPerRequest in mesos configuration)", request.getInstancesSafe(),
    ...
    public SingularityDeploy checkDeploy(SingularityRequest request, SingularityDeploy deploy) {
        ...
        if (request.isLoadBalanced()) {
            ...
            checkBadRequest(deploy.getLoadBalancerGroups().isPresent() && !deploy.getLoadBalancerGroups().get().isEmpty(),
