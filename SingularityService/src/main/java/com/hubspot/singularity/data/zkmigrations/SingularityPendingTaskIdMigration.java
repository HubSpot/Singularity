...
public class SingularityPendingTaskIdMigration extends ZkDataMigration {
  ...
  private Optional<String> getCmdLineArgs(String pendingTaskId) throws Exception {
    byte[] data = curator
      .getData()
      .forPath(ZKPaths.makePath(PENDING_TASKS_ROOT, pendingTaskId));
  ...
  public SingularityPendingTaskId createFrom(String string, long createdAt) {
    ...
    try {
      ...
      throw new InvalidSingularityTaskIdException(
        String.format("PendingTaskId %s was invalid (%s)", string, ise.getMessage())
      );
    }
    ...
    try {
      ...
      return new SingularityPendingTaskId(
        requestId,
        deployId,
        nextRunAt,
        instanceNo,
      ...
      throw new InvalidSingularityTaskIdException(
        String.format(
          "PendingTaskId %s had an invalid parameter (%s)",
          string,
          e.getMessage()
        )
      );
