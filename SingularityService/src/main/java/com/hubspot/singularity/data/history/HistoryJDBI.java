...
public interface HistoryJDBI {
  ...
  @SqlUpdate("INSERT INTO requestHistory (requestName, request, createdAt, updatedAt) VALUES (:requestName, :request, :createdAt, :createdAt)")
  void insertRequestHistory(@Bind("requestName") String requestName, @Bind("request") byte[] request, @Bind("createdAt") Date createdAt);
  
  @SqlUpdate("UPDATE requestHistory SET request = :request, updatedAt = :updatedAt WHERE requestName = :requestName")
  void updateRequestHistory(@Bind("requestName") String requestName, @Bind("request") byte[] request, @Bind("updatedAt") Date updatedAt);
  ...
  @SqlQuery("SELECT request, createdAt, updatedAt FROM requestHistory WHERE requestName = :requestName")
  List<SingularityRequestHistory> getRequestHistory(@Bind("requestName") String requestName);
  
  @Mapper(SingularityRequestHistoryMapper.class)
  @SqlQuery("SELECT request, createdAt, updatedAt FROM requestHistory WHERE requestName LIKE CONCAT('%', CONCAT(:requestNameLike, '%'))")
