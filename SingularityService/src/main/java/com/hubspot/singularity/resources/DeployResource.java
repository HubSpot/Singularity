...
public class DeployResource extends AbstractRequestResource {
  ...
  public SingularityRequestParent deploy(SingularityDeployRequest deployRequest, SingularityUser user) {
    ...
    SingularityPendingDeploy pendingDeployObj = new SingularityPendingDeploy(deployMarker, Optional.<SingularityLoadBalancerUpdate>empty(), DeployState.WAITING, deployProgress, updatedValidatedRequest);
  ...
  public SingularityRequestParent cancelDeploy(SingularityUser user, String requestId, String deployId) {
    ...
    deployManager.createCancelDeployRequest(new SingularityDeployMarker(requestId, deployId, System.currentTimeMillis(), user.getEmail(), Optional.<String>empty()));
