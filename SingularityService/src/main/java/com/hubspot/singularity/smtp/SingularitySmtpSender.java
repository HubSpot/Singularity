...
public class SingularitySmtpSender implements Managed {
  ...
  @Inject
  public SingularitySmtpSender(
    Optional<SMTPConfiguration> maybeSmtpConfiguration,
  ...
  public void queueMail(
    ...
    if (!mailSenderExecutorService.isPresent()) {
      LOG.warn(
        String.format(
          "Couldn't queue email %s because no SMTP configuration is present",
    ...
    final String realSubject = maybeSmtpConfiguration.isPresent() &&
      maybeSmtpConfiguration.get().getSubjectPrefix().isPresent()
      ? maybeSmtpConfiguration.get().getSubjectPrefix().get() + subject
      : subject;

    LOG.debug("Queuing an email to {}/{} (subject: {})", toList, ccList, realSubject);

    mailSenderExecutorService
      .get()
      .submit(() -> sendMail(toList, ccList, realSubject, body));
  }
  ...
  private Session createSession(SMTPConfiguration smtpConfiguration, boolean useAuth) {
    ...
    properties.setProperty(
      "mail.smtp.port",
      Integer.toString(smtpConfiguration.getPort())
    );
    ...
    if (useAuth) {
      ...
      return Session.getInstance(
        ...
        new SMTPAuthenticator(
          smtpConfiguration.getUsername().get(),
          smtpConfiguration.getPassword().get()
        )
      );
    } else {
