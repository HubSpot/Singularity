...
public class SingularityTaskShellCommandDispatchPoller
  ...
  @Inject
  SingularityTaskShellCommandDispatchPoller(
    TaskManager taskManager,
    SingularityConfiguration configuration,
    SingularityMesosSchedulerClient schedulerClient,
    Transcoder<SingularityTaskShellCommandRequest> transcoder
  ) {
  ...
  public void runActionOnPoll() {
    ...
    if (!schedulerClient.isRunning()) {
      LOG.warn(
        "Unable to process shell requests because scheduler driver isn't present ({} tasks waiting)",
        shellRequests.size()
      );
      return;
    }
    ...
    for (SingularityTaskShellCommandRequest shellRequest : shellRequests) {
      ...
      if (!task.isPresent() || !taskManager.isActiveTask(shellRequest.getTaskId())) {
        LOG.info(
          "Skipping shell request {} because {} didn't exist or isn't active",
          shellRequest,
          shellRequest.getTaskId()
        );
        continue;
      }

      final ExecutorID executorId = MesosProtosUtils.toExecutorId(
        task.get().getMesosTask().getExecutor().getExecutorId()
      );
      final AgentID slaveId = MesosProtosUtils.toAgentId(
        task.get().getMesosTask().getAgentId()
      );
    ...
    LOG.info(
      "Sent {} shell requests to executors in {}",
      shellRequests.size(),
