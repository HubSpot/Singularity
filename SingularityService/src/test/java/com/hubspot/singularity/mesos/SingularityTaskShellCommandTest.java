...
public class SingularityTaskShellCommandTest extends SingularitySchedulerTestBase {
  ...
  public void testTaskShellCommandPersistence() {
    ...
    // bad shell cmd
    try {
      taskResource.runShellCommand(task.getTaskId().getId(), user, new SingularityShellCommand("test-cmd", Arrays.asList("one", "two"), user));
    ...
    // bad option
    try {
      taskResource.runShellCommand(task.getTaskId().getId(), user, new SingularityShellCommand("d1", Arrays.asList("one", "two"), user));
    } catch (WebApplicationException exception) {
      assertEquals(400, exception.getResponse().getStatus());
    }

    SingularityTaskShellCommandRequest firstShellRequest = taskResource.runShellCommand(task.getTaskId().getId(), user, new SingularityShellCommand("d1", Arrays.asList("o1", "o2"), user));
    ...
    SingularityTaskShellCommandRequest secondShellRequest = taskResource.runShellCommand(task.getTaskId().getId(), user, new SingularityShellCommand("d2", Collections.<String> emptyList(), user));
