...
public class SingularityClientProvider implements Provider<SingularityClient> {
  ...
  public SingularityClientProvider setHosts(
    @Named(SingularityClientModule.HOSTS_PROPERTY_NAME) List<String> hosts
  ) {
  ...
  public SingularityClientProvider setRetryStrategy(
    @Named(SingularityClientModule.RETRY_STRATEGY) Predicate<HttpResponse> retryStrategy
  ) {
  ...
  static String getClusterMembers(CuratorFramework curator) {
    try {
      final List<String> leaders = curator
        .getChildren()
