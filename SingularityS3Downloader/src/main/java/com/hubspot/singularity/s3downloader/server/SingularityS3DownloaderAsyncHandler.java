...
public class SingularityS3DownloaderAsyncHandler implements Runnable {
  private static final Logger LOG = LoggerFactory.getLogger(
    SingularityS3DownloaderAsyncHandler.class
  );
  ...
  public SingularityS3DownloaderAsyncHandler(
  ...
  private boolean download() throws Exception {
    ...
    if (continuation.isExpired()) {
      LOG.trace(
        "Continuation expired for {}, aborting...",
    ...
    if (continuation.isExpired()) {
      LOG.info(
        "Continuation expired for {} after download, aborting...",
        artifactDownloadRequest.getTargetDirectory()
      );
      return false;
    }
    ...
    if (Objects.toString(fetched.getFileName()).endsWith(".tar.gz")) {
      ...
      artifactManager.copy(
        fetched,
        targetDirectory,
        artifactDownloadRequest.getS3Artifact().getFilename()
      );
    }

    LOG.debug(
      "Finishing request {} after {}",
      artifactDownloadRequest.getTargetDirectory(),
      JavaUtils.duration(start)
    );
  ...
  public void run() {
    ...
    try (final Context context = metrics.getDownloadTimer().time()) {
      ...
      exceptionNotifier.notify(
        String.format("Error handling download (%s)", t.getMessage()),
        t,
        ImmutableMap.of(
          "s3Bucket",
          artifactDownloadRequest.getS3Artifact().getS3Bucket(),
          "s3Key",
          artifactDownloadRequest.getS3Artifact().getS3ObjectKey(),
          "targetDirectory",
          artifactDownloadRequest.getTargetDirectory()
        )
      );
