...
public class SingularityS3DownloaderHandler extends AbstractHandler {
  ...
  private Optional<ArtifactDownloadRequest> readDownloadRequest(
    HttpServletRequest request
  ) {
    try {
      return Optional.of(
        objectMapper.readValue(request.getInputStream(), ArtifactDownloadRequest.class)
      );
    } catch (Throwable t) {
