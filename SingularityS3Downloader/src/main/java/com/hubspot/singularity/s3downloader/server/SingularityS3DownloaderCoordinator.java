...
public class SingularityS3DownloaderCoordinator implements DownloadListener {
  ...
  @Inject
  public SingularityS3DownloaderCoordinator(
    SingularityS3DownloaderConfiguration configuration,
    SingularityS3DownloaderMetrics metrics,
    Provider<ArtifactManager> artifactManagerProvider,
    @Named(
      SingularityS3DownloaderModule.ENQUEUE_EXECUTOR_SERVICE
    ) ScheduledThreadPoolExecutor downloadJoinerService,
    @Named(
      SingularityS3DownloaderModule.DOWNLOAD_EXECUTOR_SERVICE
    ) ThreadPoolExecutor downloadService,
  ...
  public void register(
    ...
    final ArtifactDownloadRequest artifactDownloadRequest
  ) {
    final DownloadJoiner downloadJoiner = new DownloadJoiner(
      continuation,
  ...
  public void notifyDownloadFinished(SingularityS3DownloaderAsyncHandler handler) {
    ...
    LOG.debug(
      "Handler for artifact {} finished download - removed {}",
      handler.getS3Artifact(),
      removed
    );
  }
  ...
  private class DownloadJoiner implements Runnable {
    ...
    private void reEnqueue() {
      LOG.debug(
        "Re-enqueueing request for {}, waiting {}, ({} active, {} queue, {} max), total time {}",
        artifactDownloadRequest.getTargetDirectory(),
      ...
      downloadJoinerService.schedule(
        this,
        configuration.getMillisToWaitForReEnqueue(),
        TimeUnit.MILLISECONDS
      );
    }

    private boolean addDownloadRequest() {
      SingularityS3DownloaderAsyncHandler existingHandler = downloadRequestToHandler.get(
        artifactDownloadRequest.getS3Artifact()
      );
      ...
      final SingularityS3DownloaderAsyncHandler newHandler = new SingularityS3DownloaderAsyncHandler(
        artifactManagerProvider.get(),
        artifactDownloadRequest,
        continuation,
        metrics,
        exceptionNotifier,
      ...
      existingHandler =
        downloadRequestToHandler.putIfAbsent(
          artifactDownloadRequest.getS3Artifact(),
          newHandler
        );
      ...
      LOG.trace(
        "Queing new downloader for {} ({} handlers, {} active threads, {} queue size, {} max) after {}",
        artifactDownloadRequest,
        downloadRequestToHandler.size(),
        downloadService.getActiveCount(),
        downloadService.getQueue().size(),
    ...
    private void handleContinuationExpired() {
      try {
        LOG.info(
          "Continuation expired for {} after {} - returning 500",
          artifactDownloadRequest,
          JavaUtils.duration(start)
        );
        ((HttpServletResponse) continuation.getServletResponse()).sendError(
            500,
            "Hit client timeout"
          );
      } catch (Throwable t) {
        LOG.warn(
          "{} while sending error after continuation for {}",
    ...
    public void run() {
      try {
        ...
        exceptionNotifier.notify(
          String.format("Error enqueuing download (%s)", t.getMessage()),
          t,
          ImmutableMap.of("targetDirectory", artifactDownloadRequest.getTargetDirectory())
        );
        try {
          ((HttpServletResponse) continuation.getServletResponse()).sendError(500);
        } catch (IOException e) {
          LOG.error(
            "Couldn't send error for {}",
            artifactDownloadRequest.getTargetDirectory(),
            e
          );
        } finally {
