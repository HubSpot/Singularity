...
If you don't have one already (e.g. your team might be running one you can use), you can easily run your own via [Docker](docker.md). Once the cluster is up and running, the API's root is available at [`http://192.168.59.103/singularity/api`](http://192.168.59.103/singularity/api) by default.

You might also be running SingularityService locally without a VM. This works too!

-----

From this point onwards we're assuming you have a running SingularityService you can use. We'll be using `http://192.168.59.103/singularity/api` as a placeholder for the root URL of the API you're using.
...
routes:
  ...
  ".*/api/.*": "http://192.168.59.103/"
  ".*/login/.*": "http://192.168.59.103/"
...
* Requests to `localhost:4001/singularity/api` are sent to the server at `192.168.59.103`.
...
You'll be prompted to input an API root. This is the service that SingularityUI will interact with for its data. Give it your `http://192.168.59.103/singularity/api` URI.

You can change the value of this at any point by typing the following in your JS console:

```javascript
localStorage.set('apiRootOverride', 'http://192.168.59.103/singularity/api')
```
